...
function menos(id_tab, fila, fuente) {
  ...
  if (tt.rows.length > 2 || fuente == 1) {
    ...
    for (xq = 0; xq <= (cc_porPesta["p-" + id_tab]); xq++) {
      calcular_detalleV2(id_tab, xq)
    }
...
function paneles_taraer_auto_hori(pkpanel, pkgrupo, xE, pkmodulo, vv2, tiempo) {
  ...
  $.ajax({
    type: 'POST',
    url: '/paneles_items',
    data: { 'csrfmiddlewaretoken': web_token, 'Id_empresa': web_Id_empresa, 'usuario': web_usuario, 'idioma': web_idioma, 'pkpanel': pkpanel, 'pkgrupo': pkgrupo, 'xE': xE, 'pkmodulo': pkmodulo, 'filtro':'' },
    beforeSend: function () {
...
function carga_charts(Response) {
  ...
  for (y = 0; y < Response["lista_charts"].length; y++) {
    ...
    if (Response["lista_charts"][y]["tipo"] == 'tabla') {
      ...
      new_tap = new_tap + '<h4>' + Response["lista_charts"][y]["titulo"] + '</h4><div style="overflow-x:auto;">'
...
function pdf_reporte(temp_pest) {
  ...
  if (Object.keys(reppuesta_rep["sum_totales"]).length > 0) {
    ...
    if(Object.keys(reppuesta_rep["sum_totales"])[0] != ''){
...
function calcular_detalle_nuevo(pestana_int, fila) {
  ...
  for (z1 = 0; z1 < Response["campos_det"].length; z1++) {
    ID_TAG = 'pd' + pestana_int + 'fff' + fila + 'ccc' + Response["campos_det"][z1]["Nombre"] + '';

    if (Response["campos_det"][z1]["TablaCampo"] == "cmptxtsimple") {
      document.getElementById(ID_TAG).value = Response["func_det"][Response["campos_det"][z1]["Nombre"]][0]["ValorPredeterminado"]
    }
    if (Response["campos_det"][z1]["TablaCampo"] == "cmpnumsimple") {
      document.getElementById(ID_TAG).value = setTwoNumberDecimal(Response["func_det"][Response["campos_det"][z1]["Nombre"]][0]["Menor"], Response["func_det"][Response["campos_det"][z1]["Nombre"]][0]["NumDecimales"])
    }
    if (Response["campos_det"][z1]["TablaCampo"] == "cmpformuladetalle") {
      document.getElementById(ID_TAG).value = setTwoNumberDecimal(0, 2)
    }
    if (Response["campos_det"][z1]["TablaCampo"] == "cmpfecha") {
      var now = new Date();
      if (Response["func_det"][Response["campos_det"][z1]["Nombre"]][0]["Tiempo"] == "Y") {
        valor_campo = now.format("Y-m-d") + 'T' + now.format("H:i:s")
      } else {
        valor_campo = now.format("Y-m-d");
      }
    }
    if (Response["campos_det"][z1]["TablaCampo"] == "cmpreferencia") {
      valor_campo = 0
      if(Response['func_det'][Response["campos_det"][z1]["Nombre"]][0][0]['predeterminado_valor'] != ''){
        
        if(Response['func_det'][Response["campos_det"][z1]["Nombre"]][0][0]['Tipo_Predeterminado'] == '0'){
          valor_campo = Response['func_det'][Response["campos_det"][z1]["Nombre"]][0][0]['predeterminado_valor']  
          document.getElementById(ID_TAG).value  = valor_campo
					buscar_referencia_detalle_enter(document.getElementById(ID_TAG))
    ...
    if (Response["campos_det"][z1]["TablaCampo"] == "cmpreferenciaadjunto") {
      valor_campo = 0
    }
    if (Response["campos_det"][z1]["TablaCampo"] == "cmpoperacion") {
      valor_campo = 0
    }
    if (Response["campos_det"][z1]["TablaCampo"] == "cmpconsolidado") {
      valor_campo = 0
    }
    if (Response["campos_det"][z1]["TablaCampo"] == "cmparchivo") {
      valor_campo = ""
    }
    if (Response["campos_det"][z1]["TablaCampo"] == "cmpnumeroaletras") {
      valor_campo = ""
    }
    if (Response["campos_det"][z1]["TablaCampo"] == "cmpdecabecera") {
      valor_campo = ""
    }
    if (Response["campos_det"][z1]["TablaCampo"] == "cmpelectronico") {
      valor_campo = 0
    }
...
function buscar_referencia_paneles(emisor) {
  ...
  cmpsenten = cmpsenten.replace(/@usuario@/g, web_usuario)
...
function buscar_referencia_detalle(campo) {
  ...
  if(Response["func_det"][cc_nombre][0][0]["orden"] != ''){
    orderby = " order by " + Response["func_det"][cc_nombre][0][0]["orden"] + " "
  }else{
      ...
      success: function (Response) {
        ...
        if (Response['cmpvalor'].length == 0) {
          ...
          for (x = (0 + ((pago_buscador - 1) * 10)); x < bus_limite; x++) {
            ...
            for (x2 = 0; x2 < orden.length; x2++) {
              if (visibles[x2] == 'S') {
                if (orden[x2] == 'imagen' || orden[x2] == 'Imagen') {
...
function grabar_elemento(pestana_int, disparador) {
  ...
  if (esta_grbando == 0) {
    ...
    esta_grbando = 1
    if(verficar_refeencias_aplicadas(pestana_int) == false){
              ...
              if (viende_calen == 1) {
                ...
                if(pre_respuesta["tipo"] == 'Nuevo'){
                  ...
                  if(pre_respuesta["plantilla_html"].length > 0){
                    ...
                    pre_respuesta["valores_subdet"] = array_subdet
                    ...
                    registro_sin_server(pre_respuesta['tabla_cab']['PkModulo'],Response["pkregistro"],2,pestana_int,1,0, pre_respuesta, array_cab, array_det, array_subdet)
                    document.getElementById('id' + pestalla).click();
                    cerrar_elemento(Response["pestalla"])

function filtro_en_panelesdb(pkpanel, pkgrupo, xE, pkmodulo) {
...
function paneles_taraer_condicion(pkpanel, pkgrupo, xE, pkmodulo) {
  filtro = document.getElementById('ppanel_filtro' + pkpanel).value


  $.ajax({
    type: 'POST',
    url: '/paneles_items',
    data: { 'csrfmiddlewaretoken': web_token, 'Id_empresa': web_Id_empresa, 'usuario': web_usuario, 'idioma': web_idioma, 'pkpanel': pkpanel, 'pkgrupo': pkgrupo, 'xE': xE, 'pkmodulo': pkmodulo, 'filtro': filtro },
    beforeSend: function () {
      var panel_est = document.getElementById("intintpestados" + pkpanel)
      panel_est.innerHTML = 'Procesando...'
    },
    success: function (Response) {


      new_tap = '<div class="mailbox-controls">'
      new_tap = new_tap + '<div class="btn-group">'
      ...
      if (Response["tipo"] == 'tabla') {
        ...
        new_tap = new_tap + '</tbody></table></div></div>'

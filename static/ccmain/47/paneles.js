...
function CambioPermisoPanel(tipo, mismo, t_panel, t_grupo, t_usuario ){
  
  'lectura', this, 2, 13,'Admin'

}

function traer_panelgrupoConfig(t_pkpanel, t_pkgrupo){
    $.ajax({
      type: 'POST',
      url: '/acc_panelesdatagrupo',
      data: { 'csrfmiddlewaretoken': web_token, 'Id_empresa': web_Id_empresa, 'usuario': web_usuario, 'idioma': web_idioma, 't_pkpanel':t_pkpanel, 't_pkgrupo':t_pkgrupo  },
      beforeSend: function () { },
      success: function (Response) {

        acc_panelesdata = Response
        data_int = '<h4>Permisos</h4><div class="input-group" style="width: 100%;">'


        data_int = data_int + '<table width="100%;font-size: 11px;" class="table table-bordered bulk_action" style="background-color: white;overflow-x:auto; font-size: 11px;"><thead><tr>'
        data_int = data_int + '<th>Usuario</th><th>Lectura</th><th>Crear</th><th>Modificar</th><th>Borrar</th>'
        data_int = data_int + '</tr>'
        data_int = data_int +'</thead><tbody>'


        for (x = 0; x < Response["p_usuarios"].length; x++) {
          data_int = data_int +'<tr>'
          data_int = data_int + '<td>'+Response["p_usuarios"][x]['Usuario']+'</td>'
          if(Response["p_usuarios"][x]['pconsultar'] == 'S'){
            data_int = data_int + '<td><input type="checkbox" onclick="CambioPermisoPanel(\'lectura\', this, '+t_pkpanel+', '+t_pkgrupo+',\''+ Response["p_usuarios"][x]['Usuario'] +'\')" checked></td>'
          }else{
            data_int = data_int + '<td><input type="checkbox" onclick="CambioPermisoPanel(\'lectura\', this, '+t_pkpanel+', '+t_pkgrupo+',\''+ Response["p_usuarios"][x]['Usuario'] +'\')"></td>'
          }
          if(Response["p_usuarios"][x]['pcrear'] == 'S'){
            data_int = data_int + '<td><input type="checkbox" onclick="CambioPermisoPanel(\'crear\', this, '+t_pkpanel+', '+t_pkgrupo+',\''+ Response["p_usuarios"][x]['Usuario'] +'\')" checked></td>'
          }else{
            data_int = data_int + '<td><input type="checkbox" onclick="CambioPermisoPanel(\'crear\', this, '+t_pkpanel+', '+t_pkgrupo+',\''+ Response["p_usuarios"][x]['Usuario'] +'\')"></td>'
          }
          if(Response["p_usuarios"][x]['pmodificar'] == 'S'){
            data_int = data_int + '<td><input type="checkbox" onclick="CambioPermisoPanel(\'modificar\', this, '+t_pkpanel+', '+t_pkgrupo+',\''+ Response["p_usuarios"][x]['Usuario'] +'\')" checked></td>'
          }else{
            data_int = data_int + '<td><input type="checkbox" onclick="CambioPermisoPanel(\'modificar\', this, '+t_pkpanel+', '+t_pkgrupo+',\''+ Response["p_usuarios"][x]['Usuario'] +'\')"></td>'
          }
          if(Response["p_usuarios"][x]['peliminar'] == 'S'){
            data_int = data_int + '<td><input type="checkbox" onclick="CambioPermisoPanel(\'eliminar\', this, '+t_pkpanel+', '+t_pkgrupo+',\''+ Response["p_usuarios"][x]['Usuario'] +'\')" checked></td>'
          }else{
            data_int = data_int + '<td><input type="checkbox" onclick="CambioPermisoPanel(\'eliminar\', this, '+t_pkpanel+', '+t_pkgrupo+',\''+ Response["p_usuarios"][x]['Usuario'] +'\')"></td>'
          }
          data_int = data_int +'</tr>'
        ...
        $('#panelDataAdmnInt').html(data_int);
...
acc_panelesdata = {}
function traer_panelConfig(t_pkpanel){

      $.ajax({
        type: 'POST',
        url: '/acc_panelesdata',
        data: { 'csrfmiddlewaretoken': web_token, 'Id_empresa': web_Id_empresa, 'usuario': web_usuario, 'idioma': web_idioma, 't_pkpanel':t_pkpanel },
        beforeSend: function () { },
        success: function (Response) {

          acc_panelesdata = Response
          data_int = '<h4>Grupos</h4><div class="input-group" style="width: 100%;">'

          for (x = 0; x < Response["p_grupos"].length; x++) {
            data_int = data_int + '<button class="btn bg-blue btn-flat margin" onclick="traer_panelgrupoConfig('+Response["p_grupos"][x]['pkpanel']+', '+Response["p_grupos"][x]['pkgrupo']+')">'+Response["p_grupos"][x]['nombre']+'</button>'
          ...
          data_int = data_int + '<div class="input-group" style="width: 100%;" id="panelDataAdmnInt"></div>'


          $('#panelDataAdmn').html(data_int);
...
function cerrar_elementopaneles(){
    $('#' + 'idpaneles').remove();
    $('#' + 'rrpaneles').remove();
  }

function paneles_acces() {
  if (web_esAdmin == 'Y') {
    plain = document.getElementById("idpaneles")
    if (plain == null) {
      ...
      $.ajax({
        ...
        success: function (Response) {
          ...
          $("#myTab").prepend('<li role="presentation" class="" style="text-align: right;"><a href="#rrpaneles" id="idpaneles"  role="tab" data-toggle="tab" aria-expanded="false"> Paneles <i class="fa fa-times" style="color: red; background: transparent;cursor: pointer;" onclick="cerrar_elementopaneles()"></i></a> </li>');

          $("#myTabContent").prepend('<div role="tabpanel" class="tab-pane fade" id="rrpaneles" aria-labelledby="idpaneles" style="background-color: transparent;"> <div style="padding-top: 5px;"> Procesando </div></div>');

          dataPanel = Response

          data_int = '<div style="padding-left: 0px;">'
          data_int = data_int + '<div class="box box-primary direct-chat direct-chat-primary">'
          data_int = data_int + '<div class="box-header with-border" style="padding-top: 0px;padding-bottom: 0px;">'
          data_int = data_int + '<div class="col-md-12">'
          data_int = data_int + '<div class="row">'

          data_int = data_int + '<div class="input-group" style="width: 100%;">'


          for (x = 0; x < Response["lista_Paneles"].length; x++) {
            data_int = data_int + '<button class="btn bg-blue btn-flat margin" onclick="traer_panelConfig('+Response["lista_Paneles"][x]['pkPanel']+')">'+Response["lista_Paneles"][x]['nombre']+'</button>'
          }
          data_int = data_int + '</div>'

          
          data_int = data_int + '<div class="col-md-12" id="panelDataAdmn">'
          data_int = data_int + '</div>'
          ...
          $('#rrpaneles').html(data_int);
          document.getElementById('idpaneles').click();
  ...
  var dd = document.getElementById('idpaneles')
  dd.click()
}
...
function cambio_dir_reget_img(envio, pkreg, pketr, pkpanel) {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    //dejar los 2 ni idea pro que ñ != ñ
    filename = filename.replace(/ñ/g, 'n')
    filename = filename.replace(/ñ/g, 'n')
  ...
  $.ajax({
    ...
    success: function (Response) {
      ...
      document.getElementById(envio.id + '_x').innerHTML = '<a href="/media/archivos/' + web_Id_empresa + '/' + filename + '" target="_blank"> ' + filename + '</a>'
      ...
      grid.parentElement.parentElement.childNodes[1].src = '/media/archivos/' + web_Id_empresa + '/' + filename      
...
function nuevo_dir_fichaCondicon(Tsenten, pketr, pkpanel, campo, dict_add, div_envio, generaTag) {
  sentencia = Tsenten.split(';')[0]
  
  var tags = document.getElementById('pfecha_int_panel'+ pkpanel)
  var user_condi = web_usuario
  for (x2 = (1); x2 < tags.childElementCount; x2++) {
    if(tags.childNodes[x2].style.backgroundColor == 'darkblue'){
      user_condi = tags.childNodes[x2].innerHTML.split(' - ')[1]
      break
    }
  }
  ...
  $.ajax({
    ...
    async: false,
    url: '/consolidado',
    data: { 'fuente': $(this).attr("value"), 'csrfmiddlewaretoken': web_token, 'Id_empresa': web_Id_empresa, 'cmpsenten': sentencia, 'tag1': '', 'tag2': '', 'usuario':user_condi},
    success: function (Response) {
        if (Response["cmpvalor"].length == 0) {
          alert(Tsenten.split(';')[1])
        }else{
          nuevo_dir_fichaCodicionPost(pketr, pkpanel, campo, dict_add, div_envio, generaTag)
        }
    }
  });
}
...
function nuevo_dir_fichaCodicionPre(pketr, pkpanel, campo, dict_add, div_envio, generaTag) {
  ...
  if(dict['dbgrupos'][div_envio]['uscrea'] == 'Todos' || dict['dbgrupos'][div_envio]['uscrea'].toLowerCase().includes('('+web_usuario.toLowerCase()+')')){
    ...
    pCondicion = dict['dbgrupos'][div_envio]['condicion']
    var cc_camp = document.getElementById('buscaPanel_' + pkpanel + '_r').value.split(',')
    var cc_val = document.getElementById('buscaPanel_' + pkpanel).tag
    var v_fecha = new Date(document.getElementById('buscaPanel_' + pkpanel + '_fecha').value)
    ...
    if(pCondicion != ''){
      pCondicion = pCondicion.replace(/@fecha@/g,  v_fecha.format("Y-m-d"))

      pCondicion = pCondicion.replace(/@pk@/g, cc_val)
      ...
      pCondicion = pCondicion.replace(/@user@/g, user_condi)

      nuevo_dir_fichaCondicon(pCondicion, pketr, pkpanel, campo, dict_add, div_envio, generaTag)
    }else{
      nuevo_dir_fichaCodicionPost(pketr, pkpanel, campo, dict_add, div_envio, generaTag)
...
function nuevo_dir_fichaCodicionPost(pketr, pkpanel, campo, dict_add, div_envio, generaTag) {
  ...
  if(dict['dbgrupos'][div_envio]['uscrea'] == 'Todos' || dict['dbgrupos'][div_envio]['uscrea'].toLowerCase().includes('('+web_usuario.toLowerCase()+')')){
    ...
    if('Fecha' in dict_add){dict_add['Fecha'] = v_fecha.format("Y-m-d")}

    if('Usuario' in dict_add){
      if(web_esAdmin == 'N'){
        dict_add['Usuario'] = web_usuario
      }else{
        var tags = document.getElementById('pfecha_int_panel'+ pkpanel)
        var t_user = web_usuario
        for (x2 = (1); x2 < tags.childElementCount; x2++) {
    ...
    $.ajax({
      type: 'POST',
      url: '/nuevo_dir_ficha',
      data: {'csrfmiddlewaretoken': web_token, 'Id_empresa': web_Id_empresa, 'usuario': web_usuario, 'idioma': web_idioma, 't_campo': campo, 't_valor': cc_val, 't_pketr': pketr, 't_dict_add': JSON.stringify(dict_add) },
      success: function (Response) {
        ...
        poner_valor_buscar_referencia_ficha(cc_busca, div_envio,  t_user)
...
function pasar_ficha_multi(){
  ...
  var cc_nombre = nom_spli[2] //elimino 'ficha' que la el count del grupo
  var pkgrupo = dict_ficha[pkpanel]['dbgrupos'][cc_grupo]['pkgrupo']
  var condicion = dict_ficha[pkpanel]['dbgrupos'][cc_grupo]['condicion']
  
  if(condicion == ""){
    pasar_ficha_multiGrabar()
  }else{
    pasar_ficha_multiCondicion(condicion, pkpanel)
  }
}



function pasar_ficha_multiCondicion(t_senten, pkpanel) {
  pCondicion = t_senten.split(';')[0]
  ...
  $.ajax({
    ...
    async: false,
    url: '/consolidado',
    data: { 'fuente': $(this).attr("value"), 'csrfmiddlewaretoken': web_token, 'Id_empresa': web_Id_empresa, 'cmpsenten': pCondicion, 'tag1': '', 'tag2': '', 'usuario': user_condi },
    success: function (Response) {
      if (Response["cmpvalor"].length == 0) {
        alert(t_senten.split(';')[1])
      }else{
        pasar_ficha_multiGrabar()
      }
    }
...
function pasar_ficha_multiGrabar() {
  ...
  for (x = (0); x <= data_gri_base.length-2; x++) {
    datos[data_gri_base[x].split(':')[0]] = data_gri_base[x].split(':')[1].replace('@fecha', v_fecha.format("Y-m-d")).replace('@usuario', user_condi)
  }
...
function click_buscador_ficha_ref_multi(campo, fila, nomcampo, pkpanel) {
  ...
  var divv = document.getElementById("vfinal_ficha_7_multi")
  data = ''
  for (x2 = (0); x2 < fila.childElementCount; x2++) {
...
function cambio_dir_reget(envio, pkreg, pketr, pkpanel) {
  ...
  if(envio.id.substring(envio.id.length-1) == 'r'){
    div_envio = envio.parentElement.parentElement.parentElement.parentElement.id.split("_")[2]
  }else{

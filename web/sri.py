...
def sri_formularios(request, Id_empresa, t_nombre): 
    db = web.con_db.SRI(request.session['conn_user'][Id_empresa],request.session['conn_pass'][Id_empresa],request.session['conn_base'][Id_empresa],request.session['conn_ip'][Id_empresa],request.session['conn_port'][Id_empresa]) 
    formularios = db.formularios(t_nombre) 
    acumulado = {}
    for formulario in formularios:
        formulario['senten'] = formulario['senten'].replace('@fecha@',request.POST.getlist('fecha')[0])
        formulario['valores'] = db.devolver_tabla(formulario['senten'])
        acumulado.update(formulario['valores'][0])
    return ({'acumulado':acumulado})



def cedulaRuc(id):    
    ##devuelve {{"identificacion":"0920028057001","nombreCompleto":"LUBKOV GUZMAN NICOLAS ALEJANDRO","tipoPersona":"PNL","codigoPersona":null}}

    # valdiadmos si ya existe dentro de Cerocodigo
    
    url = 'https://srienlinea.sri.gob.ec/sri-catastro-sujeto-servicio-internet/rest/Persona/obtenerPersonaDesdeRucPorIdentificacion?numeroRuc=' +str(id)
    response = requests.get(url)

    if response.status_code == 200:
        print(response.text)
        context = json.loads(response.text)
        context['Resultado'] = 'Si'
        return context
    else:
        print('Error:', response.status_code)
        context['Resultado'] = 'No'
        return context
